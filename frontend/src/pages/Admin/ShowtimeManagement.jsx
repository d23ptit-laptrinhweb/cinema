...
import { showtimeApi, filmApi, branchApi, roomApi } from '../../api';
...
const ShowtimeManagement = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [stRes, filmRes, branchRes, roomRes] = await Promise.all([
        showtimeApi.getAll(),
        filmApi.getAll(),
        branchApi.getAll(),
        roomApi.getAll()
  ...
  const filtered = showtimes.filter(st =>
    [st.filmName, st.roomName, st.branchName].some(f =>
              ...
              filtered.map((st, idx) => {
                ...
                return (
                  <tr key={st.showtimeId || idx}>
                    ...
                    <td style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{st.filmName || '—'}</td>
                    <td>
                      <div>
                        <div style={{ color: 'var(--text-primary)' }}>{st.roomName || '—'}</div>
                        {st.roomType && (
                          <span className="film-badge" style={{ marginTop: '3px', display: 'inline-block' }}>
