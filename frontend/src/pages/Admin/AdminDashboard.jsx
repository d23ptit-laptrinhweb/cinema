...
import { filmApi, bookingApi, branchApi } from '../../api';
...
const AdminDashboard = () => {
  const [stats, setStats] = useState({ films: 0, bookings: 0, branches: 0, revenue: 0 });
  const [recentBookings, setRecentBookings] = useState([]);
  const [allData, setAllData] = useState({ films: [], bookings: [] });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const [filmsRes, bookingsRes, branchesRes] = await Promise.allSettled([
          filmApi.getAll(),
          bookingApi.getAll(),
          branchApi.getAll(),
        ]);

        const films = filmsRes.status === 'fulfilled' ? filmsRes.value.data.result : [];
  ...
  const genreData = useMemo(() => {
    ...
    allData.bookings.forEach(b => {
      const film = allData.films.find(f => f.filmId === b.filmId || f.filmName === b.filmName);
      if (film && film.genres) {
