...
export default function FilmListPage() {
  ...
  useEffect(() => {
    ...
    const urlSearch = searchParams.get('filmName') || '';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [filmRes, genreRes] = await Promise.allSettled([
          filmApi.getAll(),
          genreApi.getAll()
        ]);

        if (filmRes.status === 'fulfilled') {
          setFilms(filmRes.value.data.result || []);
        }
  ...
  const filteredFilms = films.filter((film) => {
    // 1. Search text
    const matchSearch = film.filmName?.toLowerCase().includes(searchTerm.toLowerCase());

    // 2. Genre
    const matchGenre = selectedGenre === 'ALL'
      ? true
      : film.genres?.some(g => g.id === selectedGenre || g.genreId === selectedGenre || g.name === selectedGenre);
    // Note: adjust the some() condition depending exactly on what GenreResponse returns

    // 3. Status
    const matchStatus = statusFilter === 'ALL'
      ? true
      : film.status === statusFilter;
              ...
              {paginatedFilms.map((film) => (
                ...
                >
                  ...
                  <div className="film-info">
                    <h3 className="film-title" title={film.filmName}>{film.filmName}</h3>
